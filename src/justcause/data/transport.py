...
def load_parquet_dataset(base_url, dest_subdir):
    ...
    Takes advantage of the efficient storage format
    ...
    cov_path = get_data(COV_URL, dest_subdir, COV_FILE, download_if_missing=True)
    cov = pd.read_parquet(cov_path)
    rep_path = get_data(REP_URL, dest_subdir, REP_FILE, download_if_missing=True)
