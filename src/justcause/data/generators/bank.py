...
class MarketingData(DataProvider):
    ...
    def load_training_data(self):
        ...
        scaled_balance = minmax_scale(
            balance.reshape(-1, 1), feature_range=(-100, 10000)
        )[:, 0]
...
if __name__ == "__main__":
    ...
    utils.confounder_outcome_plot(
        data["age"], gen.y_1 - gen.y_0, dataset="marketing-age"
    )
    utils.confounder_outcome_plot(
        data["balance"], gen.y_1 - gen.y_0, dataset="marketing-balance"
    )
    utils.confounder_outcome_plot(
        data["marital_married"], gen.y_1 - gen.y_0, dataset="marketing-married"
    )
    utils.confounder_outcome_plot(
        data["job_management"], gen.y_1 - gen.y_0, dataset="marketing-manager"
    )
