...
from ...utils import simple_comparison_mean, surface_plot
...
def f4(vars):
    # high interactions
    return (
        2 * vars[0] * vars[3] * vars[5]
        + vars[0] * vars[3] * (1 - vars[5])
        + 3 * vars[7] * vars[8]
...
class ACICGenerator(DataGenerator):
    def __init__(self, params):
        ...
        self.idx_dict = {
            name: self.covariates_df.columns.get_loc(name)
            for name in config.ACIC_SELECTED_VALUES
        }
    ...
    def load_training_data(self):
        ...
        if not os.path.isfile(fname):
            self.generate_file(
                self.random, self.homogeneous, self.confounded, self.deterministic
            )
    ...
    @staticmethod
    def make_file_name(
        random, homogeneous, deterministic=False, conf=None, counterfactual=False
    ):
    ...
    def treatment_assignment(
        self, covariates, num_parents=10, relation="weak", use_parents=None, *args
    ):
        """
        ...
        if relation == "random":
            ...
            if relation == "weak":
                ...
                exp_poly = scipy.special.expit(
                    RobustScaler().fit_transform(func.reshape(-1, 1)).reshape(1, -1)[0]
                )
                return np.random.binomial(1, p=exp_poly)
            if relation == "strong":
                weight = covariates[:, self.idx_dict["dbwt"]]
                treat = weight < np.median(
                    weight
                )  # Treatment only to low birth-weight samples
        ...
        *args
        ...
        if homogeneous:
            ...
            if relation == "weak":
                ...
                weight = (
                    RobustScaler()
                    .fit_transform(covariates[:, self.idx_dict["dbwt"]].reshape(-1, 1))
                    .reshape(1, -1)[0]
                )
                return (weight < 0) + 1

    def outcome_assignment(
        self,
        covariates,
        num_parents=10,
        constant_base=False,
        ...
        *args
        ...
        if constant_base:
            ...
            y_0 = minmax_scale(y_0.reshape(-1, 1), feature_range=(1, 2)).reshape(1, -1)[
                0
            ]

        # Use same confounders for treatment effect and outcome generation
        ite = self.treatment_effect(
            covariates,
    ...
    def generate_data(self, random, homogeneous, confounded=False, deterministic=False):
        ...
        if confounded:
            ...
            ids = np.random.choice(
                self.covariates.shape[1], size=10 - len(self.idx_dict)
            )
        ...
        # Sample data
        t = self.treatment_assignment(
            self.covariates, relation=relation, use_parents=parents
        )
        ys = self.outcome_assignment(
            self.covariates,
            constant_base=False,
            homogeneous=homogeneous,
    ...
    def generate_file(self, random, homogeneous, confounded=False, deterministic=False):
        t, ys, y, y_cf = self.generate_data(
            random, homogeneous, confounded, deterministic
        )

        # Create Dataframes
        factual = pd.DataFrame(data=np.array([t, y]).T, columns=["t", "y"])
        counterfactual = pd.DataFrame(
            data=np.array([t, y, y_cf, ys[:, 1], ys[:, 0]]).T,
            columns=["t", "y", "y_cf", "y_0", "y_1"],
        )

        # Write file
        factual_file = self.make_file_name(
            random, homogeneous, deterministic, confounded, counterfactual=False
        )
        counterfactual_file = self.make_file_name(
            random, homogeneous, deterministic, confounded, counterfactual=True
        )
        factual.to_csv(factual_file)
        counterfactual.to_csv(counterfactual_file)

    def test_generation(
        self, random, homogeneous, confounded=False, deterministic=False
    ):
        t, ys, y, y_cf = self.generate_data(
            random, homogeneous, confounded, deterministic
        )
    ...
    def generate_all_files(self):
        ...
        self.generate_file(
            random=False, homogeneous=False, confounded=True
        )  # Confounders guaranteed
...
if __name__ == "__main__":
    ...
    utils.surface_plot(
        ...
        twins.y_cf[choice],
        twins.x[choice],
        name="ACIC-hetero-conf",
    )
    utils.plot_y_dist(
        twins.y[choice], twins.y_cf[choice], method_name="ACIC-hetero-conf"
    )
